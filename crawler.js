...
const MOBILE_USER_AGENT = 'Mozilla/5.0 (Linux; Android 10; Pixel 2 XL) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/113.0.0.0 Mobile Safari/537.36';
...
async function getSiteData(context, url, {
    ...
    page.on('dialog', async dialog => {
        ...
        if (!dialogClosed) {
            try {
                ...
                log(chalk.yellow(`Failed to dismiss dialog "${dialog.message()}"`), chalk.gray(error.message), chalk.gray(error.stack));
            }
        }
    });
    ...
    log(`Will wait ${extraExecutionTimeMs}ms for extra execution time.`);
    ...
    fs.writeFileSync(path.join(outputPath, `${createUniqueUrlName(url)}.html`), html);
