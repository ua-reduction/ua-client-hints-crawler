...
const TOPICS_API_HEADERS = [
    "sec-browsing-topics",
    "observe-browsing-topics",  // Topics API related
];
...
let COMBINED_HEADERS =  DEFAULT_SAVE_HEADERS.concat(RESPONSE_HEADERS, CLIENT_HINT_REQUEST_HEADERS);
const ENABLE_TOPICS_API = true;
if (ENABLE_TOPICS_API) {
    COMBINED_HEADERS = COMBINED_HEADERS.concat(TOPICS_API_HEADERS);
}
...
class RequestCollector extends BaseCollector {
     ...
     */
    constructor(additionalOptions = {saveResponseHash: true, saveHeaders: COMBINED_HEADERS}) {
        super();
        this._saveResponseHash = (additionalOptions.saveResponseHash === true);
        this._saveHeaders = COMBINED_HEADERS;
    ...
    handleResponse(data) {
        ...
        if (response.requestHeaders) {
            request.requestHeaders = normalizeHeaders(response.requestHeaders);
        }
    }
    ...
    getData({urlFilter}) {
        ...
        return this._requests
            ...
            .map(request => ({
                ...
                initiators: getAllInitiators(request.initiator),
